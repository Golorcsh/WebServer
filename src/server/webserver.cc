...
WebServer::WebServer(
  ...
  if (openLog) {
    ...
    if (isClose_) { LOG_ERROR("========== Server init error!==========") }
      ...
      LOG_INFO("src_dir_: %s", HttpConn::src_dir_)
      LOG_INFO("SqlPool num: %d, ThreadPool num: %d", connPoolNum, threadNum)
    }
  }
}
WebServer::~WebServer() {
...
void WebServer::InitEventMode_(int trigMode) {
  ...
  switch (trigMode) {
    case 0:break;
    case 1:connEvent_ |= EPOLLET;
      break;
    case 2:listenEvent_ |= EPOLLET;
      break;
    case 3:listenEvent_ |= EPOLLET;
      connEvent_ |= EPOLLET;
      break;
    default:listenEvent_ |= EPOLLET;
      connEvent_ |= EPOLLET;
      break;
  }
  HttpConn::is_et_ = (connEvent_ & EPOLLET);
}
...
void WebServer::SendError_(int fd, const char *info) {
  assert(fd > 0);
  int ret = (int) send(fd, info, strlen(info), 0);
  if (ret < 0) {
...
bool WebServer::InitSocket_() {
  ...
  SetFdNoneBlock(listenFd_);
  LOG_INFO("Server port:%d", port_)
  return true;
}
int WebServer::SetFdNoneBlock(int fd) {
  /*设置为非阻塞*/
  assert(fd > 0);
